...
if nargin<7 %what is nargain here
    threshold=1e20;
end

[sample,nvar]=size(xmean);%what's the meaning of this statement here
if distrib == 'norm'   % you only need to specify xmean & xsd
    ...
    for j=1: nvar
        ...
        s(:,j) = xmean(j) + Itqnorm(P).* xsd(j); % this can be replaced by any inverse distribution function
    end
end
...
if distrib == 'unif'  % you only need to specify xmin & xmax
    ...
    nvar=length(xmin);% why, xmin is not array or vector
    ran=rand(nsample,nvar);
    s=zeros(nsample,nvar);
    for j=1: nvar
        idx=randperm(nsample);
